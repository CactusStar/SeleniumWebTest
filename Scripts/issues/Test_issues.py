...
class Testissues(unittest.TestCase):
    data_path = './Assets/data/issues_data/issues_data.json'
    @classmethod
    def setUpClass(cls):
    ...
    def test_issues_control(self):
        ...
        self.assertEqual(Common.get_url(), self.data["pageurl"])
        ...
        self.assertEqual(self.data["created"], created_text)
        ...
        self.assertEqual(self.data["assigned"], assigned_text)
        ...
        self.assertEqual(self.data["mentioned"], mentioned_text)
        ...
        self.assertIn(self.data["open"], open_text)
        ...
        self.assertIn(self.data["closed"], closed_text)
        ...
        self.assertEqual(self.data["Visibility"]["name"], vis_text)
        ...
        self.assertEqual(self.data["Visibility"]["header"], vis_header)
        loopcount = len(self.data["Visibility"]["menulist"])
        for i in range(0, loopcount):
            self.assertEqual(self.data["Visibility"]["menulist"]["item" + str(i)], Common.find_target_elements(IssuesObjects.Visibility_MenuItems[0], IssuesObjects.Visibility_MenuItems[1])[i].text)
    ...
    def test_select_assigned(self):
        ...
        self.assertEqual(Common.get_url(), self.data["assignedurl"])
        ...
        self.assertEqual(self.data["nomatch"], content_text)
        guidance_text = Common.get_text(IssuesObjects.guidance_content[0], IssuesObjects.guidance_content[1])
        self.assertEqual(self.data["guidance"], guidance_text)
